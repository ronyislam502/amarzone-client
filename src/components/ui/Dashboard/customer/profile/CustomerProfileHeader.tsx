...
const CustomerProfileHeader = ({
  ...
  const [hasCopiedCustomerId, setHasCopiedCustomerId] = useState<boolean>(false);

  const customerDisplayName = customerProfile.name || "Customer";
  const customerFirstLetter = customerDisplayName.charAt(0).toUpperCase() || "C";
  const customerIdString = customerProfile._id || "";

  const handleCopyCustomerId = async () => {
    if (!customerIdString) return;
    try {
      await navigator.clipboard.writeText(customerIdString);
      setHasCopiedCustomerId(true);
      setTimeout(() => {
        setHasCopiedCustomerId(false);
      }, 2000);
    } catch {
  ...
  const formattedMembershipDate = customerProfile.createdAt
  ...
  const isCustomerAccountActive = !customerProfile.isDeleted;
  ...
  return (
    <div className="card relative overflow-hidden bg-[#170d2f] shadow-2xl border border-white/10 rounded-2xl sm:rounded-3xl">
      ...
      <div className="card-body relative z-10 flex-col md:flex-row items-center md:items-start justify-between gap-6 p-6 sm:p-8">
        {/* Profile Avatar and Information */}
        <div className="flex flex-col sm:flex-row items-center sm:items-start gap-5 text-center sm:text-left">
          {/* Avatar Container */}
          <div className="relative group shrink-0">
            <div className="w-24 h-24 sm:w-28 sm:h-28 rounded-2xl sm:rounded-3xl overflow-hidden border-2 border-amber-400/30 bg-[#120824] shadow-xl flex items-center justify-center">
          ...
          <div className="space-y-2">
            ...
            <p className="text-slate-300 text-xs sm:text-sm max-w-xl leading-relaxed">
              Manage your personal identity, delivery destination, and account security preferences.
            </p>

            {/* Meta Tags: Customer ID and Registration Date */}
            <div className="flex flex-wrap items-center justify-center sm:justify-start gap-3 pt-1 text-[11px] text-slate-400">
              ...
              >
                ...
                <span className="font-mono">ID: {customerIdString.slice(0, 8)}...</span>
                {hasCopiedCustomerId ? (
              ...
              <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-white/5 border border-white/10 text-slate-300">
                ...
                <span>Member since {formattedMembershipDate}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center justify-center sm:justify-end gap-3 w-full md:w-auto shrink-0">
          <button
            type="button"
