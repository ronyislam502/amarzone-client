...
import { Download, Sparkles, Package, ExternalLink } from "lucide-react";

export interface OrdersHeaderProps {
  onExportCsv?: () => void;
}

const OrdersHeader = ({ onExportCsv }: OrdersHeaderProps) => {
  return (
      ...
      <div className="card-body relative z-10 flex-col lg:flex-row lg:items-center justify-between gap-6 p-6 sm:p-7">
        <div className="space-y-2">
          ...
          <h1 className="text-2xl sm:text-3xl font-black tracking-tight text-white">
            Orders <span className="text-amber-400">Management</span>
          </h1>
          <p className="text-slate-300/80 text-xs sm:text-sm max-w-2xl leading-relaxed">
            Manage seller-fulfilled customer orders, verify pending payments, confirm shipment dispatches, and track lifecycle statuses.
          </p>
        </div>
