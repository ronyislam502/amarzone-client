...
import { Download, Sparkles, RefreshCw, PlusCircle } from "lucide-react";
...
const DepartmentsHeader = ({
  ...
  description = "Define top-level business departments, structure taxonomy hierarchies, govern catalog segregation, and maintain operational clarity.",
  badgeText = "Enterprise Architecture",
  badgeIcon = <Sparkles className="w-3.5 h-3.5" />,
  subBadgeText = "Live Department Taxonomy",
  onExportCsv,
  onRefresh,
  isRefreshing = false,
  onOpenCreateModal,
  ...
  return (
    <div className={`card relative overflow-hidden bg-dashboard-category shadow-2xl border border-white/10 rounded-2xl sm:rounded-3xl ${className}`}>
      ...
      <div className="card-body relative z-10 flex-col lg:flex-row lg:items-center justify-between gap-6 p-6 sm:p-7">
        <div className="space-y-2">
          ...
          <h1 className="text-2xl sm:text-3xl font-black tracking-tight text-white">
            {title} {highlightTitle && <span className="text-amber-400">{highlightTitle}</span>}
          </h1>
          {description && (
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          {onRefresh && (
            <button
              ...
              className="btn btn-outline btn-sm gap-2 font-bold border-white/20 text-slate-200 hover:bg-white/10 hover:text-white transition-all shadow-sm cursor-pointer rounded-xl"
            >
