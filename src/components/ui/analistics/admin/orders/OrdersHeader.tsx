...
import { Download, Sparkles, RefreshCw, ShoppingCart } from "lucide-react";
...
export const OrdersHeader: React.FC<OrdersHeaderProps> = ({
  ...
  return (
    <div className="card relative overflow-hidden bg-[#170d2f] shadow-2xl border border-white/10 rounded-2xl sm:rounded-3xl select-none">
      ...
      <div className="card-body relative z-10 flex-col lg:flex-row lg:items-center justify-between gap-6 p-6 sm:p-7">
        <div className="space-y-2">
          <div className="flex flex-wrap items-center gap-2">
            ...
            {totalOrders > 0 && (
              <span className="badge badge-outline border-white/20 text-slate-300 text-xs font-semibold">
          ...
          <h1 className="text-2xl sm:text-3xl font-black tracking-tight text-white">
            Orders & <span className="text-amber-400">Fulfillment Pipeline</span>
          </h1>
          <p className="text-slate-300/80 text-xs sm:text-sm max-w-2xl leading-relaxed">
            Monitor marketplace customer orders, vendor dispatches, platform commissions, payment verification, and delivery tracking.
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          {onRefresh && (
            <button
              ...
              className="btn btn-ghost btn-sm gap-2 font-bold text-slate-300 hover:text-white hover:bg-white/10 border border-white/10 rounded-xl cursor-pointer transition-all"
            >
