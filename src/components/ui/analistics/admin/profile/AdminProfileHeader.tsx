...
const AdminProfileHeader = ({
  ...
  const [hasCopiedAdminId, setHasCopiedAdminId] = useState<boolean>(false);

  const adminDisplayName = adminProfile.name || "Administrator";
  const adminFirstLetter = adminDisplayName.charAt(0).toUpperCase() || "A";
  const adminIdString = adminProfile._id || "";

  const handleCopyAdminId = async () => {
    if (!adminIdString) return;
    try {
      await navigator.clipboard.writeText(adminIdString);
      setHasCopiedAdminId(true);
      setTimeout(() => {
        setHasCopiedAdminId(false);
      }, 2000);
    } catch {
  ...
  const formattedTenureDate = adminProfile.createdAt
        ...
        <div className="flex flex-col sm:flex-row items-center sm:items-start gap-5 text-center sm:text-left">
          ...
          <div className="space-y-2">
            ...
            <p className="text-slate-300 text-xs sm:text-sm max-w-xl leading-relaxed">
              Manage administrator profile details, security preferences, and system-wide administrative governance.
            </p>

            {/* Meta Tags: Admin ID and Registration Date */}
            <div className="flex flex-wrap items-center justify-center sm:justify-start gap-3 pt-1 text-[11px] text-slate-400">
              ...
              >
                ...
                <span className="font-mono">ID: {adminIdString.slice(0, 8)}...</span>
                {hasCopiedAdminId ? (
              ...
              <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-white/5 border border-white/10 text-slate-300">
                ...
                <span>Admin since {formattedTenureDate}</span>
              </div>
            </div>
          </div>
        </div>
